...
{
    self = [self initWithFrame:frame horizontalPadding:8 verticalPadding:5 inputViewMinHeight:36 inputViewMaxHeight:100 type:EMChatToolbarTypeGroup];
    if (self) {
...
- (void)_setupSubviews{
    ...
    lineView.backgroundColor = RGB(187, 187, 187);
    ...
    _inputTextView.layer.borderColor = [RGB(179, 179, 179)CGColor];
    _inputTextView.layer.borderWidth = 1.0f;
    _inputTextView.returnKeyType = UIReturnKeySend;
    _inputTextView.enablesReturnKeyAutomatically = YES; // UITextView内部判断send按钮是否可以用
    _inputTextView.placeHolder = @"以角色的身份说话";
    _inputTextView.delegate = self;
    
    _previousTextViewContentHeight = [self _getTextViewContentH:_inputTextView];
    [_toolbarView addSubview:_inputTextView];
    
    UIView *lineView1 = [[UIView alloc]initWithFrame:CGRectMake(0, 44, SCREEN_WIDTH, 1)];
    lineView1.backgroundColor = RGB(187, 187, 187);
        ...
        if (!text.length && range.length == 1) {
            ...
            if (attributed[SYWordStylePointedName]) {
                ...
                if (length > 1) {
                    ...
                    int location = range.location + range.length - length.integerValue;
                    int end = location + length.intValue > attributedText.length ? attributedText.length : location + length.intValue;
                    [attributedText deleteCharactersInRange:NSMakeRange(location, end - location)];
...
- (void)composeToolSwitchBtn:(BOOL)isButtonOn{
    if (!isButtonOn) {
        if (_delegate && [_delegate respondsToSelector:@selector(chatToolbarSwitchBtn:)]) {
            [_delegate chatToolbarSwitchBtn:0];
        }
    }else{
        if (_delegate && [_delegate respondsToSelector:@selector(chatToolbarSwitchBtn:)]) {
            [_delegate chatToolbarSwitchBtn:1];
        }
    }
}

- (void)startDrama{
    [[NSNotificationCenter defaultCenter] postNotificationName:SYMessageDidDramaBtnClickNotication object:nil];
}

- (void)startScreen{
    [[NSNotificationCenter defaultCenter] postNotificationName:SYMessageDidScreenBtnClickNotication object:nil];
}

- (void)startMention{
    [[NSNotificationCenter defaultCenter] postNotificationName:SYMessageDidMentionBtnClickNotication object:nil];
}

- (void)startAlert{
    [[NSNotificationCenter defaultCenter] postNotificationName:SYMessageDidAlertBtnClickNotication object:nil];
}
