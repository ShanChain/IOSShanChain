...
//用于消息扩展属性
@property(nonatomic, strong)NSNumber *msgAttr;
...
- (void)setModel:(id<IMessageModel>)model{
    ...
    self.msgAttr = [ext objectForKey:@"msgAttr"];
    
    
    if ([self respondsToSelector:@selector(isCustomBubbleView:)] && [self isCustomBubbleView:model]) {
        if (self.msgAttr == @1) {
            [self setCustomModel:model];
        }else if(self.msgAttr == @2 || self.msgAttr == @3){
            [self setCustomModel2:model];
        }else{
            [self setCustomModel1:model];
        }
