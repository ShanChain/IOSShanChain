...
+ (void)initialize{
    ...
    cell.messageNameColor = RGB(102, 102, 102);
    cell.messageNameFont = [UIFont systemFontOfSize:14];
    cell.messageNameHeight = 20;
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier model:(id<IMessageModel>)model {
    ...
    if (self) {
        ...
        _sayLabel.textColor = RGB(255, 255, 255);
        _sayLabel.textAlignment = NSTextAlignmentCenter;
        _sayLabel.font = [UIFont systemFontOfSize:10];
        _sayLabel.layer.masksToBounds = YES;
        _sayLabel.layer.cornerRadius = 10.0f;
        _sayLabel.text = @"本人说";
        _sayLabel.backgroundColor = RGB(187, 187, 187);
        self.bubbleView.textLabel.backgroundColor = [UIColor clearColor];
...
//    _bubbleView.backgroundImageView.image = self.model.isSender ? self.sendBubbleBackgroundImage : self.recvBubbleBackgroundImage;
    ...
    UIBezierPath *maskPath = [UIBezierPath bezierPathWithRoundedRect:_bubbleView.layer.bounds byRoundingCorners:UIRectCornerTopLeft | UIRectCornerBottomLeft | UIRectCornerTopRight cornerRadii:CGSizeMake(15, 15)];
    CAShapeLayer *maskLayer = [CAShapeLayer new];
    maskLayer.frame = _bubbleView.layer.bounds;
    ...
    gradientLayer.colors = @[(__bridge id)RGB_HEX(0x5CEECA).CGColor,(__bridge id)RGB_HEX(0x3BBAC8).CGColor];
...
//    [self addConstraint:[NSLayoutConstraint constraintWithItem:self.sayLabel attribute:NSLayoutAttributeRight relatedBy:NSLayoutRelationEqual toItem:self.statusButton attribute:NSLayoutAttributeLeft multiplier:1.0 constant:-EaseMessageCellPadding]];
//    
//    [self addConstraint:[NSLayoutConstraint constraintWithItem:self.sayLabel attribute:NSLayoutAttributeTop relatedBy:NSLayoutRelationEqual toItem:self.bubbleView attribute:NSLayoutAttributeBottom multiplier:1.0 constant:0]];
...
//    [self addConstraint:[NSLayoutConstraint constraintWithItem:self.sayLabel attribute:NSLayoutAttributeRight relatedBy:NSLayoutRelationEqual toItem:self.statusButton attribute:NSLayoutAttributeLeft multiplier:1.0 constant:-EaseMessageCellPadding]];
    ...
    self.sayLabel.frame = CGRectMake(SCREEN_WIDTH/2 - 100, 20, 40, 20);
}
