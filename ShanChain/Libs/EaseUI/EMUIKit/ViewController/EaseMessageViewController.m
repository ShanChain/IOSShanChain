...
- (void)viewDidLoad {
    ...
    [[EaseBaseMessageCell appearance] setAvatarSize:50.f];
    [[EaseBaseMessageCell appearance] setAvatarCornerRadius:25.f];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([object isKindOfClass:[NSString class]]) {
        ...
        if (_delegate && [_delegate respondsToSelector:@selector(messageViewController:cellForMessageModel:indexPath:)]) {
            NSLog(@"%@",model.message.ext);
            UITableViewCell *cell = [_delegate messageViewController:tableView cellForMessageModel:model indexPath:indexPath];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([object isKindOfClass:[NSString class]]) {
        ...
        if (_dataSource && [_dataSource respondsToSelector:@selector(isEmotionMessageFormessageViewController:messageModel:)]) {
            if ([_dataSource isEmotionMessageFormessageViewController:self messageModel:model]) {
