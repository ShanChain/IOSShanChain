...
- (NSUInteger)indexOf:(NSString *)substring startFrom:(NSUInteger) index {
    NSString* test = [self substringFromIndex:index];
    NSUInteger i = [test indexOf:substring];
...
- (NSString *)substringWithFromIndex:(NSUInteger)from toIndex:(NSUInteger)to {
    NSRange range;
    range.location = from > self.length ? self.length - 1 : from;
    range.length = to > self.length ? self.length - from : to - from;
...
- (NSArray *)split:(NSString *)token limit:(NSUInteger)maxResults {
    ...
    while ([buffer contains:token]) {
        ...
        NSString* nextPart = [buffer substringWithFromIndex:0 toIndex:matchIndex];
...
- (NSString *)replaceTargtSubString:(NSString *)target withString:(NSString *)replacement {
