...
#define DSStatusPhotosMaxCount 3
#define DSStatusPhotosMaxCols(photosCount) ((photosCount == 4) ? 2 :3)
#define DSStatusPhotosW ([[UIScreen mainScreen] bounds].size.width-40)/3
...
- (void)statusPhotoOnTap:(UITapGestureRecognizer *)recognizer {
    ...
    for (int i = 0; i <count; i++){
        ...
        //设置来源于哪一个UIImageView
        [photo setSrcImageView:self.subviews[i] withSrcURL:_picUrls[i]];
...
- (void)layoutSubviews {
    ...
    if (count == 1) {
        DSStatusPhotoView *photoView = self.subviews[0];
        photoView.x = 0;
        photoView.width = SCREEN_WIDTH - 2 * KSCMargin;
        photoView.height = self.height;
    } else {
        for (int i = 0; i < count; i++){
            DSStatusPhotoView *photoView = self.subviews[i];
...
+ (CGSize)sizeWithPhotosCount:(int)photosCount type:(int)type {
    ...
    if (type == 3) {
        CGFloat photosW = SCREEN_WIDTH - 2 * DSStatusCellInset;
        CGFloat photosH = 160.0/667 * SCREEN_HEIGHT;
