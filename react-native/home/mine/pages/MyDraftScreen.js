...
class ExpandListItem extends Component {
  ...
  expandRefresh(preState) {
    if (!preState) {
      SCNetwork.post(CommonUrlMap.GET_DRAFT_LIST_BY_SPACEID, {
...
const MyDraftScreen = StackNavigator({
  MyDraft: {
    screen: MyDraft,
    navigationOptions: {
      header: ({navigation}) => {
