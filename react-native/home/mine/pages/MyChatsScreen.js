      ...
      <TouchableWithoutFeedback onPress={() => SCNavigator.pushNativePage('ChangeBrandScreen', JSON.stringify({gData: this.props.gData, data: rowData}))}>
        <View style={styles.itemContainer}>
          ...
          <View style={styles.leftText}>
            <Text style={styles.leftTitle}>{rowData.intro.substring(0, 15)}</Text>
          </View>
        </View>
      </TouchableWithoutFeedback>
    </View>)
...
const MyChatsScreen = StackNavigator({
  MyChats: {
    screen: MyChats,
    navigationOptions: {
      header: ({navigation}) => {
        return (<CommonHeaderView goBack={() => SCNavigator.pop()} isShowBack={true} navigation={navigation} title={'我的对话'}></CommonHeaderView>)
      }
    }
  }
